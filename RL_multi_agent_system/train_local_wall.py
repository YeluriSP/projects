import json
from grid_env_local_wall import PredatorPreyEnv
...
# Agents
hero_agent = DQNAgent(input_dim=10, n_actions=4, lr=1e-3, gamma=0.9)
mon_agent  = DQNAgent(input_dim=8, n_actions=4, lr=1e-3, gamma=0.9)
...
for ep in range(num_episodes):
    ...
    if wins_on_current_maze >= 75:
        print("75 terminal episodes completed on this maze. Generating a new one...")
        m, n = env.m, env.n
    ...
    # === Model saving ===
    if (ep + 1) % 50 == 0:
        torch.save(hero_agent.net.state_dict(), f"saved_models_local/hero_dqn_ep{ep+1}.pth")
        torch.save(mon_agent.net.state_dict(), f"saved_models_local/monster_dqn_ep{ep+1}.pth")
        print("Models saved.")
        print("current maze wins = ",wins_on_current_maze)
