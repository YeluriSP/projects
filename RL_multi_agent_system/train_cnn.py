import json
from grid_env_cnn import PredatorPreyEnv
from steps_cnn import DQNAgent
import numpy as np
import random
...
# Agents
hero_agent = DQNAgent(n_actions=4, grid_h=env.m, grid_w=env.n)
mon_agent  = DQNAgent(n_actions=4, grid_h=env.m, grid_w=env.n)
...
for ep in range(num_episodes):
    ...
    # === Maze change logic ===
    if wins_on_current_maze >= 50:
        print("50 terminal episodes completed on this maze. Generating a new one...")
        m, n = env.m, env.n
    ...
    print(f"Episode {ep+1} done — Hero reward={total_r_h:.2f}, Monster reward={total_r_m:.2f} and current maze wins={wins_on_current_maze}")
